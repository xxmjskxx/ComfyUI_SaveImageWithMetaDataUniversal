...
- `saveimage_unimeta/nodes/`: save node + supporting UI tooling (scanner, rule writers, extra metadata nodes, test stubs). Node tooltips have no hard character limit (ComfyUI wraps multi-line tooltips; the `filename_prefix` tooltip already exceeds 140 chars), so write clear, complete tooltips. The 140-character rule from `ruff.toml` applies to code line length only.
...
- `saveimage_unimeta/trace.py`: BFS graph traversal + sampler selection heuristics (exact match in `defs/samplers.py`, else `MetaField.SAMPLER_NAME` or `{STEPS, CFG_SCALE}` hint) plus primary-model selection (`Trace.find_model_node_id` walks upstream from the sampler's model inputs with `_PRIMARY_INPUT_PRIORITY` tie-breaking). `Trace.filter_inputs_by_trace_tree` guarantees deterministic ordering upstream of capture.
- `saveimage_unimeta/utils/`: hashing primitives (`hash.py`, single-pass AutoV1/V2/V3/SHA-256), the central AutoV1/AutoV3 JSON cache (`hash_cache.py`), LoRA/embed utilities, logging helpers (`color.py`), workflow redaction (`redaction.py`, bounded secret/path sanitization), and filename safety (`pathsafety.py`). Always use these helpers—no ad-hoc hashing/log formatting, prefer `pathresolve` for filesystem work, and route metadata/redaction and filename sanitization through the shared helpers. Hash normalization/records live in `defs/hash_values.py`.
...
5. Hashing: `saveimage_unimeta/defs/formatters.py` (via helpers in `saveimage_unimeta/utils/hash.py`) caches full SHA256 hashes in `.sha256` sidecars which are truncated to 10 chars when written to metadata; `METADATA_FORCE_REHASH=1` invalidates caches. AutoV1/AutoV3 are additionally computed in the same scan and cached in a central JSON cache (`saveimage_unimeta/utils/hash_cache.py`, at `folder_paths.get_user_directory()/.saveimage_unimeta/hash-cache.json`); the structured `Hash detail` section emits all four hashes when available. Hash log verbosity is controlled via `METADATA_HASH_LOG_MODE` and `METADATA_HASH_LOG_PROPAGATE`.
