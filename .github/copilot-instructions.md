...
`METADATA_ENABLE_TEST_NODES=1` registers lightweight stub nodes (`MetadataTestSampler`) for metadata-only test workflows so ComfyUI isn't forced to load large checkpoints.
